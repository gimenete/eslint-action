...
async function updateCheck(id, conclusion, output) {
  var annotations = output.annotations
  do {
    output.annotations = annotations.splice(0,50);
    const body = {
      name: checkName,
      head_sha: GITHUB_SHA,
      status: (annotations.length < 50 ? 'completed' : 'in_progress'),
...
async function run() {
  ...
  try {
    ...
    await updateCheck(id, 'failure', {annotations: []})
    exitWithError(err)
  }
}
